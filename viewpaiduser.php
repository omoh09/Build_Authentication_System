			...
			<div class="table-responsive">
				...
				<?php
					for ($counter = 2; $counter < $countallbills ; $counter++) {
					    ...
					    $paidString = file_get_contents("db/bills/".$currentbill);
					    $paidfObject = json_decode($paidString);
					    $amountFromDB = $paidfObject->amount;
					    $emailFromDB = $paidfObject->email;
					    $refFromDB = $paidfObject->txref;
					    $statusFromDB = $paidfObject->status;
						$finduser = finduser($emailFromDB);
						$userNames = $finduser->first_name." ".$finduser->last_name;
						if($statusFromDB == 'successful'){
